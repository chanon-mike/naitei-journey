...
const UserButton = async ({ session, accessToken }: UserButtonProps) => {
  ...
  return (
    <>
      <a className="rounded-full bg-red-400 text-white cursor-pointer" onClick={testAuth}>Test Auth Token</a>
      <a className="rounded-full bg-yellow-400 text-white cursor-pointer" onClick={fetchData}>Get User Test</a>
    </>
  )
};
